...
class TripleMeetingServiceTest : TripleMeetingTest() {
    ...
    fun `존재하지 않는 사용자에 대해 팀 유저 리스트 조회시 오류 발생`() {
        ...
        // when & then
        Assertions.assertThatThrownBy { tripleMeetingService.getMeetingTeamUserList(notExistUserId, "") }
            .isInstanceOf(UserNotFoundException::class.java)
    }
    ...
    fun `존재하지 않는 팀에 대한 팀 코드에 대해 유저 리스트 조회 시 오류를 발생`() {
        ...
        // when & then
        Assertions.assertThatThrownBy { tripleMeetingService.getMeetingTeamUserList(user.id!!, "AB19") }
    ...
    fun `팀에 속하지 않은 사용자가 팀 정보 조회 시 오류 발생`() {
        ...
        // when & then
        Assertions.assertThatThrownBy {
            tripleMeetingService.getMeetingTeamInformation(user.id!!)
        }
            .isInstanceOf(UserTeamNotFoundException::class.java)
    }
    ...
    fun `존재하지 않는 사용자가 팀 삭제를 요청 시 오류 발생`() {
        ...
        // when & then
        Assertions.assertThatThrownBy {
            tripleMeetingService.deleteMeetingTeam(notExistUserId)
        }
            .isInstanceOf(UserNotFoundException::class.java)
    }
    ...
    fun `팀에 속하지 않은 사용자가 팀 삭제를 요청 시 오류 발생`() {
        ...
        // when & then
        Assertions.assertThatThrownBy {
            tripleMeetingService.deleteMeetingTeam(user.id!!)
        }
            .isInstanceOf(UserTeamNotFoundException::class.java)
    }
