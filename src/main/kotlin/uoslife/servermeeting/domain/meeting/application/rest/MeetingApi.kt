...
class MeetingApi(
  ...
  @Operation(
      summary = "팅 결성 대기 중 간에 미팅 팀 유저 리스트 조회", description = "1대1의 경우 지원되지 않음. 1대1은 팀 유저가 본인 단독")
  @ApiResponse(
      responseCode = "200",
  ...
  fun updateMeetingTeamInformation(
      ...
      @PathVariable teamType: TeamType,
      @PathVariable isTeamLeader: Boolean,
      @RequestBody @Valid meetingTeamInformationUpdateRequest: MeetingTeamInformationUpdateRequest,
  ): ResponseEntity<Unit> {
    val userUUID = UUID.fromString(userDetails.username)
